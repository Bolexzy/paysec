...
import { ArrowRight, FileLock2, Users, EyeOff, ShieldCheck, Activity, TrendingUp } from "lucide-react";
...
interface TxRow { hash: string; type: string; party: string; ts: bigint; }
...
const Index = () => {
  const { connect, fullAddress } = useWallet();
    ...
    const load = async () => {
      try {
        ...
        const [sent, paid] = await Promise.all([
          publicClient.getLogs({ address: NOVAPAY_ADDRESS, event: PRIVATE_SENT, fromBlock: from, toBlock: latest }),
          publicClient.getLogs({ address: NOVAPAY_ADDRESS, event: INVOICE_PAID, fromBlock: from, toBlock: latest }),
        ]);
        const all: TxRow[] = [
          ...sent.map(l => ({ hash: `${l.transactionHash?.slice(0, 6)}…${l.transactionHash?.slice(-4)}`, type: "Private Send", party: `${(l.args.to as string).slice(0, 6)}…${(l.args.to as string).slice(-4)}`, ts: l.args.timestamp as bigint })),
          ...paid.map(l => ({ hash: `${l.transactionHash?.slice(0, 6)}…${l.transactionHash?.slice(-4)}`, type: "Invoice Paid", party: `${(l.args.payer as string).slice(0, 6)}…${(l.args.payer as string).slice(-4)}`, ts: l.args.timestamp as bigint })),
  ...
  return (
    <AppShell>
      ...
      <section className="relative">
        ...
        <div className="relative mx-auto max-w-5xl px-6 pb-16 pt-14 md:pt-12">
          <div className="flex flex-col items-center text-center">
            ...
            <h1 className="font-display mt-6 max-w-3xl text-balance text-4xl font-bold leading-[1.08] tracking-tight text-foreground md:text-5xl lg:text-5xl">
      ...
      {/* ── Live Overview (no wallet required) ── */}
      <section id="overview" className="mx-auto max-w-5xl px-6 pb-14">
        <p className="eyebrow mb-5"><span className="pulse-dot" />Live Network</p>
        ...
        <div className="mt-4 card-elevated overflow-hidden">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-5 py-6 text-center text-xs text-muted-foreground">No recent activity on this network.</td>
                </tr>
              ) : rows.map((r, i) => (
                <tr key={i} className="border-b border-hairline last:border-0 hover:bg-secondary/40">
                  ...
                  <td className="px-5 py-3.5 hidden sm:table-cell"><RedactedValue value={r.party} /></td>
